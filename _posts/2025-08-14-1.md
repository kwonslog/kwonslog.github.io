...
excerpt: "wandb 를 이용한 학습 과정 시각화 지표 분석"
date: 2025-08-14
last_modified_at: 2025-08-18
categories: [AI]
tags: [LLM, wandb, 시각화, 지표]
toc: true
---
...
## 인사이트
- train(학습) loss 와 eval(검증) loss 의 차이점에 대해 이해함.
  - train loss 값이 지속적으로 낮아 지더라도 eval loss 값이 낮아지지 않으면 과적합 일 수 있다.
    - 과적합은 학습한 데이터는 잘 맞추지만 새로운 데이터를 주었을때 성능이 떨어지는 현상.
      - 과적합과 학습 데이터량의 관계
        - 데이터가 적을수록 모델은 우연한 패턴·노이즈까지 규칙처럼 학습
        - 큰 모델은 자유도가 높아, 적은 데이터만으로도 훈련 손실 최소화가 가능
        - 그래서 학습 할때 loss 값이 낮아지더라도 검증 할때 loss 값이 높아 질수 있다.
      
- 손실곡면 : 수많은 벡터들을 방향과 크기에 따라 최저점을 향해 내려가는 곡선들의 모음.
  - 학습률과 그레디언트의 영향을 받는다.
  - 학습률은 설정값으로 입력하여 조절이 가능하고 그레이언트 값은 자동 계산이다.
    - 새 가중치는 = 기존 가중치 - (학습률 * 그레디언트)
  - 손실곡면은 새 가중치 값이 곡면 위의 점 1개가 되고 모든 가중치에 대한 점들이 표현된다.
    - 손실곡면은 전체 데이터 기준 가중치의 손실을 줄이는 것이 목적이다. 전체 데이터의 손실이 줄어들면 정답률이 상승하는 효과를 볼 수 있다.

## 정리
- 최소한의 데이터로 학습을 시키고 그 과정을 확인 했다.
- 시각화 한 train, eval 지표들의 경우 loss, learning_rate 등에 대한 개념적인 부분은 어느 정도 이해를 하게 되었다.
- 하지만 전체 지표들에 대해 평가를 내리고 학습 방향을 재조정 하기에는 모르는게 많다.(뭘 모르는지도 모르는 그런...)
- 좀 답답한 과정이었지만 이 정도 선에서 정리를 하고 다음 단계로 넘어가야 겠다.
